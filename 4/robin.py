...
# Example usage
input_data = read_input_file("input/convection/input.txt")
result = subprocess.run(["block_robin.exe"], capture_output=True, text=True)

path=f"output/convection/{input_data['bottom']:.6f}_{input_data['left']:.6f}.txt"
...
plt.savefig(f"robin1_{input_data['bottom']:.6f}_{input_data['left']:.6f}.png", dpi=300, bbox_inches='tight')
