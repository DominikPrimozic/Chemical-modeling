...
# Example usage
input_data = read_input_file("input/laplace/input.txt")
result = subprocess.run(["block_temperature.exe"], capture_output=True, text=True)

path=f"output/laplace/{input_data['bottom']:.6f}_{input_data['left']:.6f}.txt"
...
plt.savefig(f"laplace3_{input_data['bottom']:.6f}_{input_data['left']:.6f}.png", dpi=300, bbox_inches='tight')
