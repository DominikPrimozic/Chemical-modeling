...
# Example usage
input_data = read_input_file("input/poisson_neuman/input.txt")
result = subprocess.run(["neuman_poisson.exe"], capture_output=True, text=True)

path=f"output/poisson_neuman/{input_data['bottom']:.6f}_{input_data['left']:.6f}.txt"

data=np.loadtxt(path, delimiter="\t")

rows, cols = data.shape
new_data = np.zeros((rows + 1, cols + 2))  # Create a larger matrix
...
img = plt.imshow(new_data, cmap="coolwarm", interpolation="bicubic",extent=[0, 1, 0, 1])
...
plt.savefig(f"neumanS_{input_data['bottom']:.6f}_{input_data['source']:.6f}.png", dpi=300, bbox_inches='tight')
