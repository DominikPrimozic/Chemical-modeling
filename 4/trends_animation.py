# -*- coding: utf-8 -*-
"""
Created on Thu Apr  3 11:31:43 2025
...
for i in range(0,11):
    ...
    plt.plot(x, concentration[500,:], label=f"{ch:.1f}")

plt.title(f"At time 0.5: Vmax={values['v_max']:.1f}, Km={values['K_m']:.1f}, dy={dy:.6f}, dt={dt:.6f}")
plt.legend()
plt.xlabel("y")
plt.ylabel("z")
...
pyo.plot(fig, filename=f"animation_concentration_{values['D']:.6f}_{values['v_max']:.6f}_{values['K_m']:.6f}_{dy:.6f}_{dt:.6f}_trend.html")
"""