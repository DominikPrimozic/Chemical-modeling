...
for file_path in file_list:
    try:
        ...
        # Add circles with proper PBC handling
        for cx, cy in coords:
            shifts = [(0, 0)]  # Always plot original position
            wrapped_positions = []
            
            # Check if the circle crosses boundaries and split it
            if cx + d/2 > a/2:
                wrapped_positions.append((cx - a, cy))  # Left
            if cx - d/2 < -a/2:
                wrapped_positions.append((cx + a, cy))  # Right
            if cy + d/2 > a/2:
                wrapped_positions.append((cx, cy - a))  # Bottom
            if cy - d/2 < -a/2:
                wrapped_positions.append((cx, cy + a))  # Top

            # Handle corner cases (crossing two boundaries)
            if cx + d/2 > a/2 and cy + d/2 > a/2:
                wrapped_positions.append((cx - a, cy - a))  # Top-right to bottom-left
            if cx + d/2 > a/2 and cy - d/2 < -a/2:
                wrapped_positions.append((cx - a, cy + a))  # Bottom-right to top-left
            if cx - d/2 < -a/2 and cy + d/2 > a/2:
                wrapped_positions.append((cx + a, cy - a))  # Top-left to bottom-right
            if cx - d/2 < -a/2 and cy - d/2 < -a/2:
                wrapped_positions.append((cx + a, cy + a))  # Bottom-left to top-right

            # Draw the original and wrapped circles
            for wx, wy in wrapped_positions + [(cx, cy)]:
                x, y = create_circle(wx, wy, r=d/2)
