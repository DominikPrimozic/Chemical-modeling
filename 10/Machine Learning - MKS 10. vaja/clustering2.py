# -*- coding: utf-8 -*-
"""
Created on Fri May 30 11:19:46 2025
...
features = [col for col in df.columns if col not in ['melting_point', 'SMILES']]
...
def scale_and_pca(df_target, target_name):
    features = df_target.drop(columns=['SMILES', 'melting_point']).copy()
...
pca_boiling_df, pca_boiling, scaler_boiling = scale_and_pca(df, 'melting_point')
...
features_to_check = ['MW', 'SLogP', 'melting_point', "TPSA", "nRot", "SMR",'nHBAcc', 'nHBDon','Zagreb2']
...
pca_boiling_df = apply_kmeans(pca_boiling_df, pca_boiling.n_components_, n_clusters=8)
...
pca_boiling_df = apply_agglomerative(pca_boiling_df, pca_boiling.n_components_, n_clusters=8)
...
def find_outliers_by_distance(df, feature_cols, id_col='SMILES', label_col='melting_point', top_n=10, plot=False):
...
outlier_df = find_outliers_by_distance(df, features, id_col='SMILES', label_col='melting_point', top_n=5)
...
def plot_distance_vs_boiling(df):
    ...
    plt.scatter(df['melting_point'], df['distance_from_mean'], color='mediumseagreen', edgecolor='k', alpha=0.7)
    plt.xlabel('Melting Point')
    plt.ylabel('Distance from Mean')
    plt.title('Distance from Mean vs. Melting Point')
    plt.show()
...
plt.figure(figsize=(10, 5))
plt.scatter(df['melting_point'], df['mahalanobis_distance'], 
            color='purple', edgecolor='k', alpha=0.7)
plt.xlabel('Melting Point')
plt.ylabel('Mahalanobis Distance from Mean')
plt.title('Mahalanobis Distance vs. Melting Point')
plt.show()
...
print(euc_outliers[['SMILES', 'melting_point', 'TPSA'] + features])
print(mah_outliers[['SMILES', 'melting_point', 'TPSA'] + features])
