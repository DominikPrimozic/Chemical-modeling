...
features = [col for col in df.columns if col not in ['logS', 'SMILES']]
...
def scale_and_pca(df_target, target_name):
    features = df_target.drop(columns=['SMILES', 'logS']).copy()
...
pca_boiling_df, pca_boiling, scaler_boiling = scale_and_pca(df, 'logS')
...
features_to_check = ['MW', 'SLogP', 'logS', "TPSA", "nRot", "SMR",'nHBAcc', 'nHBDon','Zagreb2']
...
plt.title('KMeans Clusters on solubility PCA')
plt.show()
...
plt.figure(figsize=(8,6))
sns.scatterplot(data=pca_boiling_df, x='PC1', y='PC2', hue='Cluster', palette='tab10', legend=False)
plt.title('Agglomerative Clusters on solubility PCA')
plt.show()
...
def find_outliers_by_distance(df, feature_cols, id_col='SMILES', label_col='logS', top_n=10, plot=False):
...
outlier_df = find_outliers_by_distance(df, features, id_col='SMILES', label_col='logS', top_n=5)
...
def plot_distance_vs_boiling(df):
    ...
    plt.scatter(df['logS'], df['distance_from_mean'], color='mediumseagreen', edgecolor='k', alpha=0.7)
...
print(euc_outliers[['SMILES', 'logS', 'TPSA'] + features])
print(mah_outliers[['SMILES', 'logS', 'TPSA'] + features])
