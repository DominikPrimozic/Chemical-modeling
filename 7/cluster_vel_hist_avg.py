...
# -------------------------
# 🔧 USER INPUT
velocity_folder = "output/cluster/1/vel/"   # Your folder with .bin files (velocity data)
...
kinetic_energies_over_time = [0.5 * magnitudes**2 for magnitudes in velocity_magnitudes_over_time]

# Flatten the list of kinetic energies for histogram
all_kinetic_energies = np.concatenate(kinetic_energies_over_time)

# -------------------------
# 📈 Plot Histogram of Kinetic Energy Distribution
plt.figure(figsize=(8, 6))
plt.hist(all_kinetic_energies, bins=30, edgecolor='black', color='salmon', alpha=0.7, density=True)
plt.xlabel('Kinetic Energy', fontsize=12)
plt.ylabel('Probability Density', fontsize=12)
plt.tight_layout()
plt.show()
